...
log using "$logs/revision_03_birthplace_stayers.log", replace text
...
gen moved_10y = .
replace moved_10y = (codprov != cprodano) if year == 2011 & cprodano < .
replace moved_10y = (codprov != prov10) if year == 1991 & prov10 < .

tempfile birthplace_results
tempname posth
postfile `posth' str12 sample str12 outcome str20 spec str24 coefname double b se p N using "`birthplace_results'", replace
...
foreach outcome in kids2 nhijos {
    ...
    post_selected_coefs, handle(`posth') sample("census") outcome("`outcome'") spec("baseline") ///
    ...
    quietly reghdfe `outcome' treat_2 treat_3 if sexo == 6 & inrange(anac, 1920, 1950) & stayer_birthprov == 1 [aw=weight2], ///
        a(cmun##i.age_c##year##i.codprov_born) cluster(year#anac) version(5)
    post_selected_coefs, handle(`posth') sample("census") outcome("`outcome'") spec("stayers") ///
        coefs("treat_2 treat_3")

    quietly reghdfe `outcome' treat_2 treat_3 if sexo == 6 & inrange(anac, 1920, 1950) & mover_birthprov == 1 [aw=weight2], ///
        a(i.year##i.age_c##i.codprov##i.codprov_born) cluster(year#anac) version(5)
    post_selected_coefs, handle(`posth') sample("census") outcome("`outcome'") spec("movers") ///
        coefs("treat_2 treat_3")
}

preserve
collapse (mean) stayer_birthprov moved_10y [aw=weight2], by(year)
export delimited using "$out/revision_03_birthplace_shares.csv", replace
restore

postclose `posth'

use "`birthplace_results'", clear
sort outcome spec coefname
save "$out/revision_03_birthplace_results.dta", replace
export delimited using "$out/revision_03_birthplace_results.csv", replace
