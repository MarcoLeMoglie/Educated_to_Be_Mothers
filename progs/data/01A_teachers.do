...
drop profession archive  id
drop type 
save "$dir\$Output_data\purge\victims_archive_temp_new.dta",replace
...
use "$dir\$Output_data\purge\victims_archive_temp_new.dta", clear
recast str15 municip, force
replace province=lower(province)
drop if province=="francia"
...
replace codprov=17 if province =="girona" & codprov ==.
...
replace codprov=15 if province =="a coruña" & codprov ==.
...
replace codprov=38 if province =="tenerife" & codprov ==.
replace codprov=35 if province =="las palmas" & codprov ==.
replace codprov=7 if province =="illes balears" & codprov ==.
...
gen idvictim1=_n
*merge m:1 municip codprov using "$Data\INE_codes\INE_municip.dta"
reclink2 municip codprov using "$Data\INE_codes\INE_municip.dta",  idmaster(idvictim1) idusing( id_ine) gen(match) minscore(0.95)
drop id_ine CODAUTO CMUN DC idvictim1

gen type = "capital" if strpos(municip, "albacete") & codprov==2
replace type= "capital" if strpos(municip, "alicant") & codprov==3
replace type= "capital" if strpos(municip, "alacant") & codprov==3
replace type= "capital" if strpos(municip, "almeria") & codprov==4
replace type= "capital" if strpos(municip, "vitoria") & codprov==1
replace type= "capital" if strpos(municip, "oviedo") & codprov==33
replace type= "capital" if strpos(municip, "avila") & codprov==5
replace type= "capital" if strpos(municip, "badajoz") & codprov==6
replace type= "capital" if strpos(municip, "mallorca") & codprov==7
replace type= "capital" if strpos(municip, "barcelona") & codprov==8
replace type= "capital" if strpos(municip, "bilbao") & codprov==48
replace type= "capital" if strpos(municip, "burgos") & codprov==9
replace type= "capital" if strpos(municip, "caceres") & codprov==10
replace type= "capital" if strpos(municip, "cadiz") & codprov==11
replace type= "capital" if strpos(municip, "santander") & codprov==39
replace type= "capital" if strpos(municip, "castellon") & codprov==12
replace type= "capital" if strpos(municip, "ciudad real") & codprov==13
replace type= "capital" if strpos(municip, "cordoba") & codprov==14
replace type= "capital" if strpos(municip, "coruña") & codprov==15
replace type= "capital" if strpos(municip, "cuenca") & codprov==16
replace type= "capital" if strpos(municip, "san sebastian") & codprov==20
replace type= "capital" if strpos(municip, "donosti") & codprov==20
replace type= "capital" if strpos(municip, "gerona") & codprov==17
replace type= "capital" if strpos(municip, "girona") & codprov==17
replace type= "capital" if strpos(municip, "granada") & codprov==18
replace type= "capital" if strpos(municip, "guadalajara") & codprov==19
replace type= "capital" if strpos(municip, "huelva") & codprov==21
replace type= "capital" if strpos(municip, "huesca") & codprov==22
replace type= "capital" if strpos(municip, "jaen") & codprov==23
replace type= "capital" if strpos(municip, "leon") & codprov==24
replace type= "capital" if strpos(municip, "lerida") & codprov==25
replace type= "capital" if strpos(municip, "lleida") & codprov==25
replace type= "capital" if strpos(municip, "lugo") & codprov==27
replace type= "capital" if strpos(municip, "madrid") & codprov==28
replace type= "capital" if strpos(municip, "malaga") & codprov==29
replace type= "capital" if strpos(municip, "murcia") & codprov==30
replace type= "capital" if strpos(municip, "navarra") & codprov==31
replace type= "capital" if strpos(municip, "orense") & codprov==32
replace type= "capital" if strpos(municip, "ourense") & codprov==32
replace type= "capital" if strpos(municip, "palencia") & codprov==34
replace type= "capital" if strpos(municip, "palmas") & codprov==35
replace type= "capital" if strpos(municip, "logro") & codprov==26
replace type= "capital" if strpos(municip, "salamanca") & codprov==37
replace type= "capital" if strpos(municip, "tenerife") & codprov==38
replace type= "capital" if strpos(municip, "segovia") & codprov==40
replace type= "capital" if strpos(municip, "sevilla") & codprov==41
replace type= "capital" if strpos(municip, "soria") & codprov==42
replace type= "capital" if strpos(municip, "tarragona") & codprov==43
replace type= "capital" if strpos(municip, "teruel") & codprov==44
replace type= "capital" if strpos(municip, "toledo") & codprov==45
replace type= "capital" if strpos(municip, "valencia") & codprov==46
replace type= "capital" if strpos(municip, "valladolid") & codprov==47
replace type= "capital" if strpos(municip, "zamora") & codprov==49
replace type= "capital" if strpos(municip, "zaragoza") & codprov==50

rename year_2 year
drop gender
save "$dir\$Output_data\purge\victims_archive_new.dta",replace 

use "$dir\$Output_data\purge\victims_archive_new.dta", clear
*drop if year<1941
drop municip
replace type = "rest province" if missing(type)

* Total
sort codprov year type
by codprov year type: egen teachers_mun = sum(victim1)   if _merge==3
sort codprov year  

*  Male teachers
sort codprov year type male 
by codprov year type male: egen teachers_mun_male = sum(male)  if _merge==3
sort codprov year type
by codprov year type: egen teachers_mun_malee = max(teachers_mun_male)  if _merge==3
drop teachers_mun_male
rename teachers_mun_malee teachers_mun_male

drop victim1  male 
sort year codprov type  
...
duplicates drop
**
save "$dir\$Output_data\purge\teachers_newbyyear.dta", replace



use "$dir\$Output_data\purge\teachers_newbyyear.dta", clear
drop if year<1942
sort codprov type  
by codprov   type: egen teachers  = sum(teachers_mun )  
by codprov   type: egen teachers_male  = sum(teachers_mun_male ) 
drop teachers_mun* year
replace type = "municipality" if type =="capital"
duplicates drop
save "$dir\$Output_data\purge\teachers_new.dta", replace