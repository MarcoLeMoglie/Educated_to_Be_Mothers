...
gen interaction=post*teachers_pc

label var interaction "Teachers X Post 1945"
...
if `i'==1 &  "`x'"== "flag4" {
local name = "0-14"
reghdfe `x'  interaction  , a(i.id i.year ) cluster(id) 
outreg2 using $Results\onlyteachers\table_reg_female, dec(3)  label nonotes addtext( Mun. FE, YES, Prov. X Year FE, YES, Cohort, `name')  ct("`tit'") tex(frag) keep(interaction ) nocons replace
*egen idprovinceyear= group(codprov year)
*
reghdfe `x'  b1945.year##c.teachers_pc, a(i.id i.year) cluster(id) 

coefplot , keep(*.year#c.teachers_pc) vertical base omitted  yline(0, lc(red) lp(dash)) xline(16, lcolor(blue) lpattern(dash)) ms(m)  ylabel(, angle(horizotal) labsize(tiny)) leg(off)  graphregion(color(white)) msize(vsmall) levels(99 95 90) title("`name'", s(small) c(black)) ytitle("`name2'", s(vsmall)) scheme(s2mono) xlabel(, angle(45) labs(tiny))   ///
coeflabels( ///
...
1971.year#c.teachers_pc= "1971" ///
1972.year#c.teachers_pc= "1972" ///
1973.year#c.teachers_pc= "1973" ///
1974.year#c.teachers_pc= "1974" ///
1975.year#c.teachers_pc= "1975" ///
1976.year#c.teachers_pc= "1976" ///
1977.year#c.teachers_pc= "1977" ///
1978.year#c.teachers_pc= "1978" ///
1979.year#c.teachers_pc= "1979" ///
1980.year#c.teachers_pc= "1980" ///
1981.year#c.teachers_pc= "1981" ///
1982.year#c.teachers_pc= "1982" ///
1983.year#c.teachers_pc= "1983" ///
1984.year#c.teachers_pc= "1984" ///
1985.year#c.teachers_pc= "1985" ///
) ///

graph save $Results/onlyteachers/event_1940_`x'`i'.gph,replace
graph export $Results/onlyteachers/event_1940_`x'`i'.pdf,replace
...
reghdfe `x'  interaction  1.post##c.teachers_pc , a(i.id i.year) cluster(id) 
outreg2 using $Results\onlyteachers\table_reg_female, dec(3)  label nonotes addtext( Mun. FE, YES, Prov. X Year FE, YES, Cohort, `name')  ct("`tit'") tex(frag) keep(interaction ) nocons 
*egen idprovinceyear= group(codprov year)
*
reghdfe `x' b1945.year##c.teachers_pc  , a(i.id i.year) cluster(id) 
...
drop interaction  sum*
}
}
restore
cd $Results/onlyteachers/
...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
**# Regression -- Only proportion of women
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
use "$Output_data/dataset.dta", clear
...
if `i'==1 &  "`x'"== "flag4" {
local name = "0-14"
reghdfe `x'  1.post##c.sum`i' 1.post##c.sum2`i', a(i.id i.year ) cluster(id) 
outreg2 using $Results\onlypopshare\table_reg_female, dec(3)  label nonotes addtext( Mun. FE, YES, Prov. X Year FE, YES, Cohort, `name')  ct("`tit'") tex(frag) keep(interaction interaction2) nocons replace
...
graph save $Results/onlypopshare/event_1940_`x'`i'.gph,replace
graph export $Results/onlypopshare/event_1940_`x'`i'.pdf,replace
...
reghdfe `x'  interaction interaction2, a(i.id i.year) cluster(id) 
outreg2 using $Results\onlypopshare\table_reg_female, dec(3)  label nonotes addtext( Mun. FE, YES, Prov. X Year FE, YES, Cohort, `name')  ct("`tit'") tex(frag) keep(interaction interaction2) nocons 
*egen idprovinceyear= group(codprov year)
*
reghdfe `x' b1945.year##c.sum`i'  b1945.year##c.sum2`i', a(i.id i.year) cluster(id) 
